...
import { ArrowLeft, Calendar as CalendarIcon, Trash2, Plus, Printer, Info, ShieldCheck, Briefcase, TrendingUp, ChevronRight, Target, Hash, ShieldAlert, Sparkles, DollarSign, Tag } from 'lucide-react';
...
export default function OpportunityFormPage() {
  ...
  const { totalFcv } = useMemo(() => {
    ...
    return { totalFcv: finalNrc + finalMrc * watchedContractMonths };
  ...
  useEffect(() => {
    if (opportunityData && !isFormLoaded.current) {
      form.reset({ ...opportunityData, currency: opportunityData.currency || 'USD', closeDate: new Date(opportunityData.closeDate), requestDate: new Date(opportunityData.requestDate), offerSentDate: opportunityData.offerSentDate ? new Date(opportunityData.offerSentDate) : undefined, contactId: opportunityData.contactId || '', salesforceId: opportunityData.salesforceId || '', lineItems: opportunityData.lineItems || [], attachments: opportunityData.attachments || [], generalDiscountPercentage: opportunityData.generalDiscountPercentage || 0, applyDiscountToNrc: opportunityData.applyDiscountToNrc || false, applyDiscountToMrc: opportunityData.applyDiscountToMrc || false, reason: opportunityData.reason || '', competition: opportunityData.competition?.join(', ') || '', risk: opportunityData.risk || 'C-Low', isPlanned: opportunityData.isPlanned || false, opportunityType: opportunityData.opportunityType || 'New Business', projectManagerEmail: opportunityData.projectManagerEmail || '', contractReferenceId: opportunityData.contractReferenceId || '', grossMarginPercentage: opportunityData.grossMarginPercentage || 0, grossMarginAmount: opportunityData.grossMarginAmount || 0, valcomAuthorized: opportunityData.valcomAuthorized || false, clientVerified: opportunityData.clientVerified || false, contractSigned: opportunityData.contractSigned || false, complianceChecked: opportunityData.complianceChecked || false });
  ...
  async function onSubmit(values: OpportunityFormData) {
    if (!user) return;
    const dataToSave = { ...values, competition: values.competition ? values.competition.split(',').map((s) => s.trim()) : [] };
    try {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <Card>
                ...
                <CardContent className="space-y-6 p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField control={form.control} name="title" render={({ field }) => (<FormItem className="md:col-span-1"><FormLabel>{t('Dashboard.recentOpportunities.opportunityHeader')} <span className="text-red-500 ml-0.5">*</span></FormLabel><FormControl><Input placeholder={t('Forms.opportunityTitlePlaceholder')} {...field} disabled={isLocked} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={form.control} name="salesforceId" render={({ field }) => (<FormItem><FormLabel className="flex items-center gap-2"><Target className="h-3 w-3" />Salesforce ID</FormLabel><FormControl><Input placeholder="006..." {...field} disabled={isLocked} /></FormControl><FormMessage /></FormItem>)} />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField control={form.control} name="risk" render={({ field }) => (<FormItem><FormLabel>{t('Forms.risk')} <span className="text-red-500 ml-0.5">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={isLocked}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent>{riskOptions.map(opt => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
                    <FormField control={form.control} name="opportunityType" render={({ field }) => (<FormItem><FormLabel>{t('Forms.opportunityType')} <span className="text-red-500 ml-0.5">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={isLocked}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent>{typeOptions.map(opt => <SelectItem key={opt} value={opt}>{opt}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <FormField control={form.control} name="projectManagerEmail" render={({ field }) => (<FormItem><FormLabel>{t('Forms.projectManagerEmail')}</FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={isLocked}><FormControl><SelectTrigger className="bg-white"><SelectValue placeholder="Seleccionar Ingeniero..." /></SelectTrigger></FormControl><SelectContent>{engineers.map(e => (<SelectItem key={e.uid} value={e.email}>{e.displayName}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
                    <FormField control={form.control} name="isPlanned" render={({ field }) => (<FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 bg-muted/5"><div className="space-y-0.5"><FormLabel className="text-base">{t('Forms.isPlanned')}</FormLabel></div><FormControl><Switch checked={field.value} onCheckedChange={field.onChange} disabled={isLocked} /></FormControl></FormItem>)} />
                  </div>
                  <FormField control={form.control} name="description" render={({ field }) => (<FormItem><FormLabel>{t('Forms.description')}</FormLabel><FormControl><Textarea placeholder={t('Forms.descriptionPlaceholder')} {...field} className="h-24" disabled={isLocked} /></FormControl><FormMessage /></FormItem>)} />
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
                  <FormField control={form.control} name="clientId" render={({ field }) => (<FormItem><FormLabel>{t('Dashboard.recentOpportunities.clientHeader')} <span className="text-red-500 ml-0.5">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={!isNew || isLocked || !!clientIdFromQuery}><FormControl><SelectTrigger><SelectValue placeholder={t('Forms.selectClient')} /></SelectTrigger></FormControl><SelectContent>{clients.map((client) => <SelectItem key={client.id} value={client.id}>{client.name}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="contactId" render={({ field }) => (<FormItem><FormLabel>{t('Forms.referenceContact')}</FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={!watchedClientId || isLocked}><FormControl><SelectTrigger><SelectValue placeholder={t('Forms.selectContact')} /></SelectTrigger></FormControl><SelectContent>{contacts.map((contact) => <SelectItem key={contact.id} value={contact.id}>{contact.name}</SelectItem>)}{contacts.length === 0 && <SelectItem value="none" disabled>Sin contactos registrados</SelectItem>}</SelectContent></Select><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="contractReferenceId" render={({ field }) => (<FormItem className="md:col-span-2"><FormLabel>{t('Forms.contractReference')}</FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={!watchedClientId || isLocked}><FormControl><SelectTrigger><SelectValue placeholder="Seleccionar contrato vigente (opcional)" /></SelectTrigger></FormControl><SelectContent><SelectItem value="none">-- Sin referencia --</SelectItem>{clientContracts?.map((contract) => <SelectItem key={contract.id} value={contract.id}>{contract.publicId} ({contract.type})</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                    <FormField control={form.control} name="stage" render={({ field }) => (<FormItem><FormLabel>{t('Dashboard.recentOpportunities.stageHeader')} <span className="text-red-500 ml-0.5">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={isLocked}><FormControl><SelectTrigger><SelectValue placeholder={t('Forms.selectStage')} /></SelectTrigger></FormControl><SelectContent>{stages.map((stage) => <SelectItem key={stage} value={stage}>{t(`Stages.${stage}`)}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)} />
                    <FormField control={form.control} name="probability" render={({ field }) => (<FormItem><FormLabel>{t('Forms.probability')} ({field.value}%)</FormLabel><FormControl><Slider min={0} max={100} step={5} value={[field.value]} onValueChange={(value) => field.onChange(value[0])} disabled={isLocked} /></FormControl><FormMessage /></FormItem>)} />
                    <FormField control={form.control} name="isTender" render={({ field }) => (<FormItem className="flex flex-row items-center gap-x-3 space-y-0 pt-8"><FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} disabled={isLocked} /></FormControl><FormLabel className="font-normal">{t('Forms.isTender')}</FormLabel></FormItem>)} />
                  </div>
              ...
              {/* SECCIÓN: VALIDACIONES DE PROCESO (Movido antes de Adjuntos) */}
              <Card className="border-primary/20 bg-primary/5">
                <CardHeader className="border-b bg-white/50">
                  <CardTitle className="text-sm font-bold uppercase tracking-wider flex items-center gap-2">
